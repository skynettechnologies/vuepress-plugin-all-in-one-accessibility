# All in One Accessibility®: VuePress Plugin
...
```bash
npm i vuepress-plugin-all-in-one-accessibility
```

### Via yarn

```bash
yarn add vuepress-plugin-all-in-one-accessibility
```

## Setup

Add the following code in your .vuepress/config.ts file:

import { AllinOneAccessibilityPlugin } from 'vuepress-plugin-all-in-one-accessibility'
